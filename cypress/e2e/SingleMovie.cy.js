describe('Movie Knight single movie user flow', () => {
  ...
  it("Should show a different url than the address bar when a movie is clicked", () => {
    cy.get("a").eq(0).click()
    cy.intercept("https://rancid-tomatillos.herokuapp.com/api/v2/movies/436270", {
      method: "GET",
      fixture: "../fixtures/SingleMovie.json"
  ...
  it('should show an error message when there is a network problem', () => {
    ...
    cy.get('h4').contains("Couldn't communicate with the server. Please try again later.")
  })
